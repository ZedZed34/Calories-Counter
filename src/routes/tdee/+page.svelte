<script>
  ...
  import { bmr, tdee } from '$lib/utils/calories';
  ...
  function handleCalculate(event) {
    ...
    if (!userData.sex || !userData.age || !userData.heightCm || !userData.weightKg || !userData.activityKey) {
...
<svelte:head>
  <title>TDEE Calculator (Metric)</title>
  <meta name="description" content="Calculate TDEE using activity multipliers (metric)." />
</svelte:head>
...
<div class="shell">
  ...
  <main>
    ...
    <UserForm showAdvanced={false} on:calculate={handleCalculate} />
    {#if showResult}
      <p style="margin-top:.5rem">Your TDEE (maintenance): <strong>{tdeeValue} kcal/day</strong></p>
    {:else}
      <div class="placeholder">
        <p>Click "Get Result" to calculate your Total Daily Energy Expenditure!</p>
      </div>
    {/if}
  </main>
</div>
